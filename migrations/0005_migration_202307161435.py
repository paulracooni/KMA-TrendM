...
class Embedding(peewee.Model):
    ...
    type = CharField(column_name='reference', max_length=32)
    news = snapshot.ForeignKeyField(column_name='news', index=True, model='news', null=True)
    keyword = snapshot.ForeignKeyField(column_name='keyword', index=True, model='keyword', null=True)
    summary = snapshot.ForeignKeyField(column_name='summary', index=True, model='trendmsummary', null=True)
    class Meta:
...
class TrendMImpact(peewee.Model):
    ...
    news = snapshot.ForeignKeyField(column_name='news', index=True, model='news')
    summary = TextField()
    valueable = BooleanField(column_name='published', default=False)
    impact = FloatField()
    usage = DoubleField()
    class Meta:
...
def forward(old_orm, new_orm):
    trendmsummary = new_orm['trendmsummary']
    return [
        # Apply default value '' to the field trendmsummary.title,
        trendmsummary.update({trendmsummary.title: ''}).where(trendmsummary.title.is_null(True)),
    ]
