...
def forward(old_orm, new_orm):
    embedding = new_orm['embedding']
    return [
        # Check the field `embedding.input` does not contain null values,
    ]
...
def backward(old_orm, new_orm):
    embedding = new_orm['embedding']
    return [
        # Apply default value '' to the field embedding.type,
        embedding.update({embedding.type: ''}).where(embedding.type.is_null(True)),
    ]
