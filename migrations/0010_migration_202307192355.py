...
def forward(old_orm, new_orm):
    log = new_orm['log']
    return [
        # Apply default value datetime.datetime(2023, 7, 19, 23, 55, 34, 336303) to the field log.datetime,
        log.update({log.datetime: datetime.datetime(2023, 7, 19, 23, 55, 34, 336303)}).where(log.datetime.is_null(True)),
    ]
...
def backward(old_orm, new_orm):
    log = new_orm['log']
    return [
        # Apply default value '' to the field log.category,
        log.update({log.category: ''}).where(log.category.is_null(True)),
    ]
